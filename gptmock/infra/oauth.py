...
class OAuthHTTPServer(http.server.HTTPServer):
    ...
    def exchange_code(self, code: str) -> tuple[AuthBundle, str]:
        ...
        bundle = AuthBundle(api_key=None, token_data=token_data, last_refresh=last_refresh_str)
        return bundle, f"{URL_BASE}/success"
    ...
    def persist_auth(self, bundle: AuthBundle) -> bool:
        ...
        if bundle.api_key:
            auth_json_contents["OPENAI_API_KEY"] = bundle.api_key
...
class OAuthHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if not state or not hmac.compare_digest(state, self.server.state):
        ...
        if auth_bundle.api_key:
            auth_json_contents["OPENAI_API_KEY"] = auth_bundle.api_key
